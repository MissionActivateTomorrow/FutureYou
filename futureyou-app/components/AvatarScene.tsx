...
// 3-step cartoon toon gradient (dark shadow → mid → bright highlight)
function useToon() {
  return useMemo(() => {
...
function CartoonAvatar({ mood, accentColor }: CartoonAvatarProps) {
  ...
  // Mood-based eye sizing + arm raise
  ...
  return (
    <group ref={groupRef} scale={1.55} position={[0, -0.25, 0]}>
      ...
      <group ref={headRef} position={[0, 0.78, 0]}>
        {/* Main head sphere – slightly vertically squished */}
        <mesh scale={[1, 1.05, 1]}>
          <sphereGeometry args={[0.34, 32, 32]} />
          <meshToonMaterial color={SKIN} gradientMap={gradientMap} />
        </mesh>

        {/* Hair cap – scenario color */}
        ...
        <group position={[-0.12, 0.04, 0.30]}>
          ...
          {/* Pupil shine */}
          <mesh position={[0.016, 0.018, 0.055]}>
            <circleGeometry args={[0.016, 8]} />
            <meshBasicMaterial color={WHITE} />
          </mesh>
          {/* Happy squint line (extra top lid) */}
          {isHappy && (
            <mesh position={[0, 0.05, 0.055]} rotation={[0, 0, 0.1]}>
        ...
        {mood === 'stressed' && (
          // Flat line
          <mesh position={[0, -0.14, 0.31]} rotation={[Math.PI / 2, 0, 0]}>
        ...
        {mood === 'struggling' && (
          ...
          <mesh position={[0, -0.12, 0.31]} rotation={[0, 0, Math.PI]}>
            <torusGeometry args={[0.06, 0.016, 8, 20, Math.PI]} />
            <meshBasicMaterial color={DARK} />
          </mesh>
        )}
      ...
      {/* ── LEFT ARM ── */}
      <group
        position={[-0.36, 0.20, 0]}
        rotation={[0, 0, isHappy ? 0.65 : isSad ? 0.1 : 0.3]}
      >
        <mesh>
...
function ScenarioProps({ scenario }: { scenario: Scenario }) {
  ...
  if (scenario.id === 'D') return (
    <>
      ...
      {/* Palm tree — deterministic angles, no Math.random */}
      <group position={[-4.5, -1.18, -4]}>
        ...
        {/* 6 palm fronds fanning out at fixed angles */}
        {[0, 1, 2, 3, 4, 5].map((i) => {
...
export default function AvatarScene({ scenario, onAvatarClick, interactive = true }: AvatarSceneProps) {
  return (
    <Canvas
    ...
    >
      ...
      <Float speed={1.0} rotationIntensity={0.08} floatIntensity={0.12}>
        <group onClick={onAvatarClick}>
          <CartoonAvatar mood={scenario.avatarMood} accentColor={scenario.accentColor} />
        </group>
      </Float>
